...
type ContentItem = {
 ...
 const parseToolToSystemPrompt = (messages: Message[], tools: Tool[]) => {
  ...
  ## ${name}
    ...
    messages = messages.map(message =>
        message.role === 'system' ? { ...message, content: systemPrompt + prompt } : message
    );
...
export function parseAssistantMessage(assistantMessage: string,tools:Tool[]) {
  ...
  function convertToOpenAIFormat(input: (TextMessage | ToolCall)[]) {
